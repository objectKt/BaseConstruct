...
public class SoundPlayer {
    ...
    public static void init() {
        try {
            ...
            new Thread(() -> {
                soundPoolMap.clear();
                soundPoolMap.put(SOUND_ID_LEFTLIGHT, mSoundPool.load(app, R.raw.zx, 1));
                soundPoolMap.put(SOUND_ID_RADAR, mSoundPool.load(app, R.raw.radar, 1));//注意，这里 hash表里 记录
                soundPoolMap.put(SOUND_ID_SAFETYBELT, mSoundPool.load(app, R.raw.safety_belt, 1));//注意，这里 hash表里 记录
                LogCat.d("音频文件预加载完成");
            }).start();
    ...
    public static synchronized void play(Type type, String fileName) {
        LogCat.d("play---type=" + type.name() + ", fileName=" + fileName + "leftTurnPlay=" + leftTurnPlay +
    ...
    public static void stopSafetyBelt() {
        LogCat.d("stopSafetyBelt----radarPlay=" + radarPlay + ", turnSignalOpen=" + SoundPlayer.turnSignalOpen());
    ...
    private static void player(String key) {
        try {
            ...
            LogCat.d("player----soundId=" + soundId + ", streamID=" + streamID);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
