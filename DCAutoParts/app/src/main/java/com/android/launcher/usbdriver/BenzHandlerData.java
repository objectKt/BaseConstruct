...
//        LogUtils.printI(TAG,"id="+id +", data="+data);
...
//                break;
            ...
            case "75":
                ...
                break;
            case "76":
                disposeEngineFailureData(data);
                break;
            case "77":
                disposeAirbagFailureData(data);
                break;
            default:
                break;
        }
    ...
    //打开安全气囊故障灯显示
    private static void disposeAirbagFailureData(String data) {
        try {
            String onStatus = data.substring(2);
            LogUtils.printI(TAG, "disposeAirbagFailureData---onStatus=" + onStatus);

            if (BinaryEntity.Value.NUM_1.getValue().equals(onStatus)) {
                SPUtils.putBoolean(App.getGlobalContext(), SPUtils.SHOW_AIRBAG_FAILURE, true);
                LivingService.operateOriginMeter = true;
            } else {
                SPUtils.putBoolean(App.getGlobalContext(), SPUtils.SHOW_AIRBAG_FAILURE, false);
    ...
    //打开发动机故障灯显示
    private static void disposeEngineFailureData(String data) {
        try {
            String onStatus = data.substring(2);
            LogUtils.printI(TAG, "disposeEngineFailureData---onStatus=" + onStatus);

            if (BinaryEntity.Value.NUM_1.getValue().equals(onStatus)) {
                SPUtils.putBoolean(App.getGlobalContext(), SPUtils.SHOW_ENGINE_FAILURE, true);
                LivingService.operateOriginMeter = true;
            } else {
                SPUtils.putBoolean(App.getGlobalContext(), SPUtils.SHOW_ENGINE_FAILURE, false);
                LivingService.operateOriginMeter = false;
            }
