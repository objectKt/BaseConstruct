...
//        }else{
        ...
        if (color == 0) {
            outerSmallArcPaint.setColor(getResources().getColor(R.color.color_lib_oil));
        } else {
...
//        canvas.drawArc(mSmallRectFArc, 120 - degree, degree, false, outerSmallArcPaint);
    ...
    private void getColor(float oil) {
        if (oil < 0.13f || new BigDecimal(oil).compareTo(new BigDecimal("0.13")) == 0) {
            color = R.color.color_lib_red;
        } else {
            color = R.color.color_lib_green;
        }
    }
    ...
    public void setOil(float oil) {
        ...
        Log.v(TechOilMeterView.class.getSimpleName(), "degree=" + degree);
        invalidate();
    }
}