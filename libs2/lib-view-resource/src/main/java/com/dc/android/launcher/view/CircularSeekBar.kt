package com.dc.android.launcher.view
...
class CircularSeekBar @JvmOverloads constructor(
     ...
     */
    var circleStyle : Paint.Cap = Paint.Cap.ROUND
        set(style) {
            field = style
            initPaints()
            recalculateAll()
    ...
    init {
        val attrArray = context.theme.obtainStyledAttributes(attrs , R.styleable.CircularSeekBar , defStyleAttr , defStyleRes)
        initPaints()
    ...
    private fun initAttributes(attrArray : TypedArray) {
        circleXRadius = attrArray.getDimension(R.styleable.CircularSeekBar_cs_circle_x_radius ,
            DEFAULT_CIRCLE_X_RADIUS
        )
        circleYRadius = attrArray.getDimension(R.styleable.CircularSeekBar_cs_circle_y_radius ,
            DEFAULT_CIRCLE_Y_RADIUS
        )
        pointerStrokeWidth = attrArray.getDimension(R.styleable.CircularSeekBar_cs_pointer_stroke_width ,
            DEFAULT_POINTER_STROKE_WIDTH
        )
        pointerHaloWidth = attrArray.getDimension(R.styleable.CircularSeekBar_cs_pointer_halo_width ,
            DEFAULT_POINTER_HALO_WIDTH
        )
        pointerHaloBorderWidth = attrArray.getDimension(R.styleable.CircularSeekBar_cs_pointer_halo_border_width ,
            DEFAULT_POINTER_HALO_BORDER_WIDTH
        )
        circleStrokeWidth = attrArray.getDimension(R.styleable.CircularSeekBar_cs_circle_stroke_width ,
            DEFAULT_CIRCLE_STROKE_WIDTH
        )
        val circleStyleAttribute =
            attrArray.getInt(R.styleable.CircularSeekBar_cs_circle_style , DEFAULT_CIRCLE_STYLE)
        circleStyle = Paint.Cap.values()[circleStyleAttribute]
        pointerColor = attrArray.getColor(R.styleable.CircularSeekBar_cs_pointer_color ,
            DEFAULT_POINTER_COLOR
        )
        pointerHaloColor = attrArray.getColor(R.styleable.CircularSeekBar_cs_pointer_halo_color ,
            DEFAULT_POINTER_HALO_COLOR
        )
        pointerHaloColorOnTouch = attrArray.getColor(R.styleable.CircularSeekBar_cs_pointer_halo_color_ontouch ,
            DEFAULT_POINTER_HALO_COLOR_ONTOUCH
        )
        circleColor =
            attrArray.getColor(R.styleable.CircularSeekBar_cs_circle_color , DEFAULT_CIRCLE_COLOR)
        circleProgressColor = attrArray.getColor(R.styleable.CircularSeekBar_cs_circle_progress_color ,
            DEFAULT_CIRCLE_PROGRESS_COLOR
        )
        circleFillColor = attrArray.getColor(R.styleable.CircularSeekBar_cs_circle_fill ,
            DEFAULT_CIRCLE_FILL_COLOR
        )
        pointerAlpha = Color.alpha(pointerHaloColor)
        pointerAlphaOnTouch = attrArray.getInt(R.styleable.CircularSeekBar_cs_pointer_alpha_ontouch ,
        ...
        max = attrArray.getInt(R.styleable.CircularSeekBar_cs_max , DEFAULT_MAX).toFloat()
        progressActual =
            attrArray.getInt(R.styleable.CircularSeekBar_cs_progress , DEFAULT_PROGRESS).toFloat()
        customRadii = attrArray.getBoolean(R.styleable.CircularSeekBar_cs_use_custom_radii ,
            DEFAULT_USE_CUSTOM_RADII
        )
        maintainEqualCircle = attrArray.getBoolean(R.styleable.CircularSeekBar_cs_maintain_equal_circle ,
            DEFAULT_MAINTAIN_EQUAL_CIRCLE
        )
        moveOutsideCircle = attrArray.getBoolean(R.styleable.CircularSeekBar_cs_move_outside_circle ,
            DEFAULT_MOVE_OUTSIDE_CIRCLE
        )
        isLockEnabled = attrArray.getBoolean(R.styleable.CircularSeekBar_cs_lock_enabled ,
            DEFAULT_LOCK_ENABLED
        )
        disablePointer = attrArray.getBoolean(R.styleable.CircularSeekBar_cs_disable_pointer ,
            DEFAULT_DISABLE_POINTER
        )
        isNegativeEnabled = attrArray.getBoolean(R.styleable.CircularSeekBar_cs_negative_enabled ,
            DEFAULT_NEGATIVE_ENABLED
        )
        isInNegativeHalf = false
        disableProgressGlow = attrArray.getBoolean(R.styleable.CircularSeekBar_cs_disable_progress_glow ,
            DEFAULT_DISABLE_PROGRESS_GLOW
        )
        hideProgressWhenEmpty = attrArray.getBoolean(R.styleable.CircularSeekBar_cs_hide_progress_when_empty ,
            DEFAULT_CS_HIDE_PROGRESS_WHEN_EMPTY
        )

        // Modulo 360 right now to avoid constant conversion
        startAngle = (360f + attrArray.getFloat(R.styleable.CircularSeekBar_cs_start_angle ,
            DEFAULT_START_ANGLE
        ) % 360f) % 360f
        endAngle = (360f + attrArray.getFloat(R.styleable.CircularSeekBar_cs_end_angle ,
            DEFAULT_END_ANGLE
        ) % 360f) % 360f
        ...
        // Modulo 360 right now to avoid constant conversion
        pointerAngle = (360f + attrArray.getFloat(R.styleable.CircularSeekBar_cs_pointer_angle ,
    ...
    private fun initPaints() {
        ...
        if (!disableProgressGlow) {
            ...
            circleProgressGlowPaint.maskFilter = BlurMaskFilter(PROGRESS_GLOW_RADIUS_DP * DPTOPX_SCALE , BlurMaskFilter.Blur.NORMAL)
        }
...
//            }
    ...
    @Suppress("UNUSED_VALUE" , "JoinDeclarationAndAssignment")
    ...
    override fun onRestoreInstanceState(state : Parcelable) {
        ...
        circleStyle = Paint.Cap.values()[savedState.getInt("circleStyle")]
