...
object SerialPortTTLDecoder {
    ...
    fun decodeBuffer(buffer: ByteArray): String {
        val bytes = buffer + "0x00".toByte()
        if (bytes.isEmpty()) {
            return ""
        }
        ...
        while (byteBuf.readableBytes() > 0) {
            ...
            if (String.format("%02X", findBytesStart) != FRAME_HEAD) {
                ...
                if (byteBuf.readableBytes() < FRAME_SIZE_AT_LEAST) {
                    ...
                    byteBuf.clear()
                    val frameHex = UtilByteArray.toHeXX(frameBytes)
                    LogCat.i("TTL 解码完整结果：$frameHex")
                    // 掐头去尾
