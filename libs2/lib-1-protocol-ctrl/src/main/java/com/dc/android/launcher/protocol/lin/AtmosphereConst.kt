...
object AtmosphereConst {
    ...
    val singleColorB0: List<String> = mutableListOf<String>().apply {
        val arrange = HUtils.ByteX.byteToUInt(0x3F.toByte())
        for (id in 0..arrange) {
            add(HUtils.ByteX.byteToHex(id.toByte()))
        }
    }

    // 氛围灯 10 个主题色  B0 = （0x50 ~ 0x59） ,B2 = 0x64 ,B6 = 0x3F
    val themeColorB0: List<String> = mutableListOf<String>().apply {
        val start = HUtils.ByteX.byteToUInt(0x50.toByte())
        val end = HUtils.ByteX.byteToUInt(0x59.toByte())
        for (id in start..end) {
            add(HUtils.ByteX.byteToHex(id.toByte()))
        }
    }
