...
public class VerticalSlider extends View {
    ...
    private void init(AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            mThumbRadius = arr.getDimensionPixelSize(R.styleable.VerticalSlider_vs_thumb_radius, mThumbRadius);
            mTrackFgThickness = arr.getDimensionPixelSize(R.styleable.VerticalSlider_vs_track_fg_thickness, mTrackFgThickness);
            mTrackBgThickness = arr.getDimensionPixelSize(R.styleable.VerticalSlider_vs_track_bg_thickness, mTrackBgThickness);
            arr.recycle();
        }
    }
    ...
    private int resolveAttrColor(String attrName, int defaultColor) {
        ...
        @SuppressLint("DiscouragedApi") int attrRes = getResources().getIdentifier(attrName, "attr", packageName);
        if (attrRes <= 0) {
            return defaultColor;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int trackPadding = mTrackBgThickness > mTrackFgThickness ? (mTrackBgThickness - mTrackFgThickness) >> 1 : 0;
        ...
        canvas.drawCircle(getPaddingLeft() + leftOffset + mThumbRadius, getPaddingTop() + mThumbRadius + (1 - mProgress) * height + trackPadding, mThumbRadius, mThumbFgPaint);
    }
